...
#mark_api = '8c4bdf9f48f7526d684119ec1e6fbc46'
...
def trigger_rest_Dashboard_API(plant_code,json_info_api,  email , proxyDict):
    
    
    
    API = "https://azapp-11804-p-002.azurewebsites.net/notifications/create"
    
    response = requests.post(API,data = json.loads(json_info_api),  proxies=proxyDict)
...
@scheduler.task('interval', id='DarkSky Alerts', seconds=3600, misfire_grace_time=1800)
def get_darksky_alerts():
    # Read current weather table
    conn_dl, curs_dl = connect_greenplum(lake_specs)
    table_name = "abs_weather_alerts_tbl"
    ...
    #request_options = '?exclude=currently,minutely,hourly,daily,flags'
    
    request_options = '?exclude=minutely,hourly,daily,flags'
    df_plants = []
    p = 'SHAP'
    for p in plant_locs:
        logger.info('Getting Weather Alerts for %s' % p)
        ...
        if darksky_response.status_code == 200:
            ...
            try:
                weather_alerts = darksky_json['alerts']
                df_alerts = pd.DataFrame()
                for wa in weather_alerts:
                    wa = {k.lower(): v for k, v in wa.items()}
                    df = pd.DataFrame(wa)
                    df_alerts = pd.concat([df_alerts, df])
                df_alerts['plant_id'] = p
                df_alerts['plant_code'] = plant_locs[p][0]
                
                if df_alerts.shape[0] > 0:
                    try:
                        
                        if weather_alerts:
                            logger.info("Weather Alerts are Available "+ p)
                            for wa in weather_alerts:
                    
                                
                                description =wa['description'] if wa.get('description') else 'NA'
                                
                                description= description.translate(str.maketrans({'\n': '', '#': '', '/':'' }))
                                
                                title ="Title : "+ wa['title'] if wa.get('title') else 'NA'
                                severity = "Severity : "+ wa['severity'] if wa.get('severity') else 'NA'
                                #time = datetime.datetime.utcfromtimestamp(wa['time']).strftime('%Y-%m-%d') if wa.get('time') else 'NA'
                                #expires = datetime.datetime.utcfromtimestamp(wa['expires']).strftime('%Y-%m-%d') if wa.get('expires') else 'NA'
                                
                                expires_API = datetime.datetime.utcfromtimestamp(wa['expires']).strftime('%Y%m%d%H%M') if wa.get('expires') else 'NA'
                                
                                text_notification = "|".join([title,severity, description])
                                logger.info("Notification is Prepared..."+ p)
                                
                                # Create JSON
                                
                                json_dict =  {"plant_code": plant_locs[p][0],
                                              "notification_text":text_notification ,"notification_type": "cloud",
                                              "user_id": email,
                                              "expires":expires_API }
                                
                                
                                json_info_api = json.dumps(json_dict)
                                
                                # Calling DashBoard API
                                                          
    
                                
                                response = trigger_rest_Dashboard_API(plant_locs[p][0], json_info_api, email ,proxyDict)
                                
                                
                                if response:
                                    logger.info("Notification is generated for Plant Code "+ p+" with response "+ str(response.status_code))
                                else:
                                    logger.info("Notification is not generated for Plant Code "+ p+" with response "+ str(response.status_code))
                        else:
                            
                            logger.info("Weather Alerts are not available in JSON "+ str(p))
                    except:
                        
                        logger.info("Error in Notification generation "+ p)
                        
                df_plants.append(df_alerts)
            except:
                logger.info("No Current Alerts for %s" % p)
                weather_alerts = {}
        else:
            logger.info("Invalid Response %d" % darksky_response.status_code)
    # Merge plant records if we got any alerts
    if df_plants:
        ...
        logger.info("Writing Weather Alerts Table: %s" % (df.shape,))
        write_frame_to_pg(df, table_name, lake_specs)
    else:
        logger.info("No Alerts to Update")
...
if __name__ == '__main__':
    ...
    # Logging
    output_file = 'darksky_weather_alerts.log'

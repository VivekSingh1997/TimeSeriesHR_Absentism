...
@scheduler.task('interval', id='DarkSky Hourly Weather', seconds=3600, misfire_grace_time=1800)
def get_darksky_weather():
    # Read current weather table
    conn_dl, curs_dl = connect_greenplum(lake_specs)
    table_name = "abs_weather_tbl"
    ...
    request_options = '?exclude=minutely,hourly,daily,flags,alerts'
    df_plants = []
    for p in plant_locs:
        logger.info('Getting Weather Snapshot for %s' % p)
        ...
        if darksky_response.status_code == 200:
            ...
            try:
                logger.info("Getting Current Weather for %s" % p)
                weather_dict = darksky_json['currently']
            except:
                logger.info("No Current Weather for %s" % p)
                weather_dict = {}
            weather_dict = {k.lower(): v for k, v in weather_dict.items()}
            df_plant = pd.DataFrame([weather_dict], columns=weather_dict.keys())
            df_plant['plant_id'] = p
    ...
    logger.info("Writing Weather Table: %s" % (df.shape,))
...
if __name__ == '__main__':
    ...
    # Logging
    output_file = 'darksky_weather.log'
